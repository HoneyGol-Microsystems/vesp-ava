...
module ava_wb (
    ...
    output  logic                       pcm_fifo_we,
    output  logic [31:0]                pcm_fifo_di,
    input   logic                       pcm_fifo_empty,
    input   logic                       pcm_fifo_full,
    input   logic                       pcm_busy,
    ...
    always_comb begin : device_select
        ...
        unique case (wb.adr[20:19])
            ...
            ADR_PCM_FIFO:   pcm_fifo_sel = 1'b1;
        endcase
    end
    ...
    always_ff @( posedge wb.clk_i ) begin : operation_start
        if (wb.rst_i | ~wb.cyc) begin // Reset or unset cyc clears all pending operations.
            ...
            pcm_fifo_latency_counter <= (reg_latency_counter  << 1) | (wb.stb & pcm_fifo_sel);
        end
    end
    ...
    always_comb begin : output_handling
        ...
        if (reg_latency_counter[$size(reg_latency_counter)-1]) begin
            ...
            wb.ack      = 1'b1;
            wb.dat_o    = vram_do;
        end else if (pcm_fifo_latency_counter[$size(pcm_fifo_latency_counter)-1]) begin
            wb.ack      = 1'b1;
            wb.dat_o    = 'h0;
        end
    end
    ...
    always_comb begin : pcm_fifo_control
        pcm_fifo_we = pcm_fifo_sel & wb.cyc && wb.stb && wb.we;
        pcm_fifo_di = wb.dat_i;
    end

    always_comb begin : register_addr_ff
        reg_offset = wb.adr[3:2];
    end

    always_ff @( posedge wb.clk_i ) begin : register_read_proc // Output FF to simulate one clock cycle latency.
        reg_out <= regs[reg_offset * 32 +: 32];
    end 

    always_ff @(posedge wb.clk_i) begin : register_write_proc
        regs.pcm_status.reserved     <= 'h0;
        regs.pcm_status.fifo_full    <= pcm_fifo_full;
        regs.pcm_status.fifo_empty   <= pcm_fifo_empty;
        regs.pcm_status.fifo_busy    <= pcm_busy;
        ...
        if (wb.rst_i) begin
            ...
            if (reg_offset == 2'b01) begin
                regs[reg_offset * 32 +: 32] <= wb.dat_i | {pcm_fifo_empty & regs.interrupts.pcm_empty_irq_en, vblank_start & regs.interrupts.vblank_irq_en};
            end else if (reg_offset != 2'b10) begin // PCM status is read-only by software.
                regs[reg_offset * 32 +: 32] <= wb.dat_i;
            end
        end else begin
            regs.interrupts <= regs.interrupts | {pcm_fifo_empty & regs.interrupts.pcm_empty_irq_en, vblank_start & regs.interrupts.vblank_irq_en};
        end
    end
